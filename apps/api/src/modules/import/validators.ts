...
export function mapPgErrorToFriendlyRu(code: unknown, context: ImportPgContext = 'generic'): string {
    ...
    // P3 (код-аудит 2026-06-14): частые ошибки импорта — раньше давали невнятное
    // «ошибка вставки» и сбрасывали весь батч.
    if (code === '23502') return 'не заполнено обязательное поле';
    if (code === '22001') return 'значение слишком длинное для поля';
    if (code === '22P02' || code === '22007') return 'некорректный формат значения (число/дата)';
