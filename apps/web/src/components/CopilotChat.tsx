...
export function CopilotChat() {
    ...
    return (
        <div className={`fixed right-4 bottom-4 z-40 ${widthClass} bg-white border border-slate-200 rounded-xl shadow-2xl flex flex-col`}
            ...
            <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-100">
                ...
                <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-slate-900 truncate">AI Со-пилот</p>
                    <p className="text-[10px] text-slate-400 truncate">Диспетчерский ассистент</p>
                </div>
                <button
                    type="button"
            ...
            {/* History dropdown */}
            {historyOpen && (
                <div className="border-b border-slate-100 max-h-48 overflow-y-auto bg-slate-50">
                    <button
                        type="button"
                        onClick={startNew}
                    ...
                    {history.length === 0 && (
                        <p className="px-3 py-2 text-xs text-slate-400">История пуста</p>
                    )}
                    {history.map((conv) => (
                        <button
                            key={conv.id}
                            ...
                            className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-100 border-t border-slate-100 ${
                                conversationId === conv.id ? 'bg-indigo-50' : ''
                            }`}
                        >
                            <p className="font-medium text-slate-700 truncate">{conv.title || 'Без темы'}</p>
                            ...
                            {turn.toolCalls.length > 0 && (
                                <div className="space-y-1 mb-2">
                                    {turn.toolCalls.map((tc) => (
                                        <div key={tc.toolUseId} className="rounded-md border border-slate-200 bg-white">
                                            <button
                                                type="button"
                                                onClick={() => toggleToolCall(turn.id, tc.toolUseId)}
                                                className="w-full flex items-center gap-1.5 px-2 py-1 text-[10px] font-mono text-slate-700 hover:bg-slate-50"
                                            >
                                            ...
                                            {tc.open && (
                                                <div className="px-2 pb-2 text-[10px] font-mono text-slate-600 space-y-1">
                                                    <div>
            ...
            {/* Input */}
            <form onSubmit={handleSubmit} className="border-t border-slate-100 p-2 flex items-end gap-2">
                <textarea
                    value={input}
                    ...
                    disabled={busy}
                    className="flex-1 resize-none text-xs px-2 py-1.5 border border-slate-200 rounded-md bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-300 disabled:opacity-50"
                />
                <button
                    type="submit"
