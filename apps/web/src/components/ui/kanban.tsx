...
export function KanbanColumn({
    ...
    return (
        <div
            ...
            onDrop={handleDrop}
            className={cn(
                'flex-shrink-0 w-[280px] xl:w-[320px] rounded-xl overflow-hidden border border-slate-200/70 transition-all duration-200',
                toneColumnBg[tone],
        ...
        >
            ...
            {/* Header */}
            <div className="px-3 py-2.5 flex items-center justify-between bg-white/70 backdrop-blur-sm border-b border-slate-200/60">
            ...
            <div className="px-2 py-2 flex flex-col gap-2 min-h-[180px] max-h-[calc(100vh-340px)] overflow-y-auto">
                {empty
                    ? (emptyState ?? (
                        <div className="flex-1 flex items-center justify-center text-center text-xs text-slate-400 py-8 px-2 rounded-lg border border-dashed border-slate-200 bg-white/40">
            ...
            {onQuickAdd && !empty && (
                <div className="px-2 pb-2">
                    <button
                        type="button"
                        onClick={onQuickAdd}
                        className="w-full inline-flex items-center justify-center gap-1.5 h-8 rounded-lg text-xs font-medium text-slate-500 hover:text-slate-800 hover:bg-white/80 transition-colors"
                    >
...
export function KanbanCard({ id, fromCol, onClick, children, className, disabled }: KanbanCardProps) {
    ...
    return (
        <div
            ...
            onClick={onClick}
            className={cn(
                'group relative bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-150',
        ...
        >
            {!disabled && (
                <div className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition-opacity text-slate-300 pointer-events-none">
...
export function ViewTabs<T extends string>({ value, onChange, options, className, ...rest }: ViewTabsProps<T>) {
    return (
        <div
            ...
            className={cn('inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white p-1', className)}
        >
            {options.map((opt) => {
                const Icon = opt.icon;
