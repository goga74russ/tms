...
async function downloadAttachmentAuth(apiPath: string, filename: string) {
    ...
    if (!res.ok) throw new Error('РќРµ СѓРґР°Р»РѕСЃСЊ СЃРєР°С‡Р°С‚СЊ С„Р°Р№Р»');
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
...
function StatusBadge({ status }: { status: string }) {
    const config: Record<string, { color: string; label: string; icon: React.ReactNode }> = {
        ...
        medical_check: {
            ...
            label: 'РњРµРґРѕСЃРјРѕС‚СЂ',
            icon: <Clock className="w-3.5 h-3.5" />,
        },
        technical_check: {
            color: 'bg-orange-100 text-orange-700 border-orange-200',
            label: 'РўРµС…РѕСЃРјРѕС‚СЂ',
            icon: <Clock className="w-3.5 h-3.5" />,
        },
        issued: {
            ...
            <Card className="w-full max-w-lg mx-4 shadow-xl max-h-[90vh] overflow-y-auto">
                ...
                <CardContent className="space-y-4">
                            ...
                            <div>
                                ...
                                {resolvedVehicleProfile && (
                                    <div className="mt-1 inline-flex rounded-full bg-indigo-50 px-2.5 py-1 text-[11px] font-semibold text-indigo-700">
                                        РџР›: {resolvedVehicleProfile.displayLabel}
                                    </div>
                                )}
                                <div className="mt-2 flex flex-wrap gap-2">
                                ...
                                {trailerInfo && (
                                    <p className="text-xs text-slate-400">
                                        РџСЂРёС†РµРї: {trailerInfo.plateNumber}
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4 space-y-3">
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p className="text-sm font-semibold text-slate-900">Р”РѕСЃСЊРµ СЂРµР№СЃР°</p>
                                </div>
                                <span className="inline-flex rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-indigo-700">
                                    {dossier.summary?.hasWaybill ? 'РџР› РµСЃС‚СЊ' : 'РџР› РЅРµС‚'}
                                </span>
                            </div>
                            <div className="grid grid-cols-2 gap-2 text-xs text-slate-600">
                                <div className="rounded-xl bg-white px-3 py-2">Р—Р°СЏРІРѕРє: {dossier.summary?.orderCount ?? 0}</div>
                                <div className="rounded-xl bg-white px-3 py-2">РўРЎ: {dossier.summary?.hasVehicle ? 'РґР°' : 'РЅРµС‚'}</div>
                                <div className="rounded-xl bg-white px-3 py-2">РџСЂРёС†РµРї: {dossier.summary?.hasTrailer ? 'РґР°' : 'РЅРµС‚'}</div>
                                <div className="rounded-xl bg-white px-3 py-2">РџР›: {dossier.summary?.hasWaybill ? 'РґР°' : 'РЅРµС‚'}</div>
                            </div>
                            <div className="rounded-xl bg-white px-3 py-2 text-xs text-slate-500">
                                {dossier.parties?.length || 0} СѓС‡Р°СЃС‚РЅРёРєРѕРІ вЂў {dossier.orders?.length || 0} Р·Р°СЏРІРѕРє РІ РґРѕСЃСЊРµ
                            </div>
                        </div>
                    )}
                    ...
                    <div className="p-3 bg-slate-50 rounded-xl space-y-3">
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                <p className="text-xs text-slate-500 font-semibold uppercase tracking-wide">Р’Р»РѕР¶РµРЅРёСЏ</p>
                                <p className="text-sm text-slate-600">Р¤Р°Р№Р»С‹ Рё СЃРєР°РЅС‹, РїСЂРёРєСЂРµРїР»С‘РЅРЅС‹Рµ Рє РїСѓС‚РµРІРѕРјСѓ Р»РёСЃС‚Сѓ.</p>
                            </div>
                            <label className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-white border border-slate-200 text-sm font-medium text-slate-700 hover:bg-slate-100 cursor-pointer">
                                ...
                                {uploadingAttachment ? 'Р—Р°РіСЂСѓР·РєР°...' : 'Р—Р°РіСЂСѓР·РёС‚СЊ'}
                                <input
                                    type="file"
                                    className="hidden"
                        ...
                        {waybill.attachments && waybill.attachments.length > 0 ? (
                            ...
                            <div className="flex items-center gap-2 text-sm text-slate-500">
                                ...
                                Р’Р»РѕР¶РµРЅРёР№ РїРѕРєР° РЅРµС‚
                            </div>
                        )}
                    </div>

                    <div className="text-xs text-slate-400 pt-2">
                        Р’С‹РґР°РЅ: {new Date(waybill.issuedAt).toLocaleString('ru-RU')}
                        {waybill.closedAt && ' Р—Р°РєСЂС‹С‚: ' + new Date(waybill.closedAt).toLocaleString('ru-RU')}
                    </div>
...
export default function WaybillsPage() {
    ...
    const handleUploadAttachment = async (file: File) => {
        ...
        try {
            ...
            setToast({ message: 'Р’Р»РѕР¶РµРЅРёРµ Р·Р°РіСЂСѓР¶РµРЅРѕ', type: 'success' });
        } catch (err: any) {
            setToast({ message: err.message || 'РќРµ СѓРґР°Р»РѕСЃСЊ Р·Р°РіСЂСѓР·РёС‚СЊ РІР»РѕР¶РµРЅРёРµ', type: 'error' });
        } finally {
    ...
    const handleDeleteAttachment = async (attachmentId: string) => {
        ...
        try {
            ...
            await openDetail(detailWaybill.id);
            setToast({ message: 'Р’Р»РѕР¶РµРЅРёРµ СѓРґР°Р»РµРЅРѕ', type: 'success' });
        } catch (err: any) {
            setToast({ message: err.message || 'РќРµ СѓРґР°Р»РѕСЃСЊ СѓРґР°Р»РёС‚СЊ РІР»РѕР¶РµРЅРёРµ', type: 'error' });
        }
    };
    ...
    const handleDownloadAttachment = async (attachment: WaybillAttachment) => {
        ...
        try {
            ...
            setToast({ message: err.message || 'РќРµ СѓРґР°Р»РѕСЃСЊ СЃРєР°С‡Р°С‚СЊ РІР»РѕР¶РµРЅРёРµ', type: 'error' });
        }
    };
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 text-slate-700 font-medium text-xs">
                                            <div className="flex flex-col gap-0.5">
                                                ...
                                                {false && (
                                                    <span className="inline-flex w-fit rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-700">
                                                        РџР›: {getVehicleProfile(vehicleMap[wb.vehicleId].bodyType).displayLabel}
                                                    </span>
                                                )}
                                                {false && (
                                                ...
                                                {trailerMap[wb.vehicleId] && (
                                                    <span className="text-[11px] text-slate-400">
                                                        + РїСЂРёС†РµРї {trailerMap[wb.vehicleId].plateNumber}
                                                    </span>
                                                )}
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1">
                                                ...
                                                <button
                                                    ...
                                                    className="p-1 rounded hover:bg-sky-100 transition-colors" title="РџСЂРµРґРїСЂРѕСЃРјРѕС‚СЂ Р­РўСЂРќ"
                                                >
