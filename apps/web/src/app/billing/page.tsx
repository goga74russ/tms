...
export default function BillingPage() {
    ...
    return (
        <div className="max-w-6xl mx-auto p-6 space-y-6">
            ...
            {/* Current plan card */}
            {sub && (
                <div className="bg-gradient-to-br from-brand-50 to-white border border-brand-100 rounded-xl p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4">
                    <div className="min-w-0">
                        <p className="text-xs uppercase tracking-wide text-brand-700 font-semibold">Текущий тариф</p>
                        <h2 className="text-xl font-bold text-neutral-900 mt-1">{PLAN_LABELS[sub.plan.id] ?? sub.plan.nameRu}</h2>
                        <div className="flex items-center gap-3 mt-2 flex-wrap">
                            {sub.subscription && (
                                <span className="inline-flex items-center px-2 py-0.5 bg-white border border-brand-200 text-brand-700 text-xs rounded-full">
                        ...
                        <div className="mt-2 flex items-center gap-3 md:justify-end flex-wrap">
                            {sub.subscription?.status === 'active' && sub.plan.priceMonthlyKopecks > 0 && (
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 text-xs font-medium text-brand-700 hover:text-brand-900 underline-offset-2 hover:underline"
                                >
                ...
                <section className="bg-white border border-neutral-200 rounded-xl p-5">
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                        ...
                        <button
                            type="button"
                            onClick={handleManagePayment}
                            className="inline-flex items-center gap-1.5 text-xs font-semibold text-brand-700 hover:text-brand-900 underline-offset-2 hover:underline shrink-0"
                        >
                            <Wallet className="w-3.5 h-3.5" />
                            ...
                            return (
                                <div
                                    ...
                                    className={`rounded-xl border bg-white p-5 flex flex-col gap-3 ${
                                        isCurrent ? 'border-brand-400 ring-2 ring-brand-100' : 'border-neutral-200'
                                    }`}
                                >
                                    <div>
                                    ...
                                    {isCurrent ? (
                                        ...
                                        <button
                                            ...
                                            className="inline-flex items-center justify-center gap-1.5 w-full px-3 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium hover:bg-brand-700 disabled:opacity-50"
                                        >
                                ...
                                {history.map((p) => {
                                    ...
                                    return (
                                        <tr key={p.id} className="hover:bg-neutral-50">
                                            ...
                                            <td className="px-4 py-2.5">
                                                {p.receiptUrl ? (
                                                    <a href={p.receiptUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-brand-600 hover:underline text-xs">
            ...
            >
                <div className="space-y-3">
                    <button
                        ...
                        className="w-full text-left rounded-xl border border-neutral-200 hover:border-brand-300 hover:bg-brand-50/40 transition-colors p-4 flex items-start gap-3"
                    >
