...
## 🔧 Спринт 10: Бизнес-инварианты и P0-блокеры (DONE)
**Статус:** ✅ DONE | **Обновлено:** 13.03.2026
**Цель:** Закрыть разрывы между рейсом, путевым листом, заказом и инвойсом.

> [!NOTE]
> Sprint 10 закрыт по коду. В этом спринте доведены ключевые инварианты между рейсом, путевым, заказом, биллингом и клиентским доступом.

### ✅ 1. Тарификация multi-order рейсов
- `TarificationService` переведен на рабочий контур через `trip_orders`.
- Убраны критичные legacy-зависимости, мешавшие корректному расчету multi-order рейсов.

### ✅ 2. Синхронизация статусов завершения
- При `TripStatus.COMPLETED` связанные `orders` переводятся в `delivered`.

### ✅ 3. Освобождение активов при отмене
- При `TripStatus.CANCELLED` освобождаются связанные активы и снимаются зависания отмененных рейсов.

### ✅ 4. Запрет обхода выпуска
- Прямой переход в `WAYBILL_ISSUED` защищен guard-проверкой наличия корректного путевого листа.

### ✅ 5. Легитимность предрейсовых допусков
- Для предрейсовых мед- и техосмотров `tripId` обязателен.
- Повторное использование допуска вне рейса исключено на уровне service-логики.

### ✅ 6. Финансовый стоп по оригиналам документов
- В `trips` добавлен признак получения оригиналов документов.
- Биллинг не формируется без подтвержденного комплекта первички.

### ✅ 7. RBAC для клиентского портала
- Права роли `client` для `Order` приведены к read-only модели.

### ✅ 8. Переход чтения рейсов на `trip_orders`
- `getTripById()` и ключевые чтения рейса используют нормализованную many-to-many связь через `trip_orders`.
- `orders.tripId` остается только как переходный миграционный хвост.

---

## 🧱 Спринт 11: Жесткие проверки и документный контур (IN PROGRESS)
**Статус:** 🔄 В работе | **Обновлено:** 13.03.2026
