...
## 📦 Спринт 9: Операционный процесс и Диспетчерская (DISCUSS)
**Статус:** 🟡 Требует обсуждения | **Обновлено:** 12.03.2026
**Цель:** Привести систему к реальным операционным процессам перевозчика — путевые листы, заявки, диспетчерская.

> [!NOTE]
> Все пункты требуют обсуждения и уточнения перед реализацией.

### 1. Расширение карточки заявки — грузовые параметры
Добавить в модель и UI заявки:
- **Количество мест** (`seatCount`) — сколько единиц груза (паллет, коробок, рулонов и т.п.)
- **Объём груза** (`cargoVolume`, м³) — общий объём отправления
- **Разрешение грузить 2-3 ярус** (`multiTierAllowed`, boolean + `maxTiers`: 1/2/3) — можно ли штабелировать груз по ярусам
- **Данные по прицепу** (`trailerInfo`) — тип прицепа, госномер прицепа, грузоподъёмность прицепа, привязка к ТС

> 💡 Сейчас в заявке есть `weight` и `cargoType`. Нужно расширить модель `orders` новыми колонками + обновить форму создания заявки + отображение в карточке.

### 2. Окно на день (дневное планирование)
- **Дневное расписание диспетчера** — На экране(новая заявка) убрать окно от\окно до и оставить выбор 1 дня.

### 3. Статус заявки: «Подтверждена → В работе»
- изменить название статуса с Подтверждена → В работе

> 💡 Сейчас стейт-машина заявок: `new → assigned → in_transit → delivered → completed / cancelled`.

### 4. Инцидент по мед/техосмотру
- Если результат медосмотра или техосмотра **не OK** → автоматически создаётся **инцидент**
- Инцидент блокирует или не блокирует выпуск на линию (водитель/ТС не допускаются к рейсу)(В зависимости от тяжести инценденнта)
- Модель `incidents` — тип (med/tech), связь с `inspections`, статус (open/resolved), комментарий
- Уведомление диспетчеру/логисту (Telegram + in-app)

> 💡 Сейчас осмотры (`inspections`) фиксируют результат, но не блокируют рейс. Нужна бизнес-логика блокировки.
...
Расширить карточку ТС (`vehicles`) и/или водителя:
- **Топливная карта** (`fuelCardNumber`, `fuelCardProvider`) — номер, оператор (Газпромнефть, Лукойл, РН-Карт и т.п.)
- **Пропуск** (`passNumber`, `passZone`, `passExpiry`) — зональный пропуск (МКАД, ТТК, СК и т.п.)
- **Транспондер** (`transponderNumber`, `transponderProvider`) — T-Pass, «Мир» и т.п.
- Уведомления об истечении сроков действия (как в предиктивном ТО)

> 💡 Часть данных привязана к ТС (транспондер, пропуск), часть — к водителю (топливная карта). Нужно определить модель.
...
- Путевой лист может иметь **нескольких водителей** (смена за рулём: водитель 1, водитель 2)
- Таблица `waybill_drivers` (many-to-many): `waybillId`, `driverId`, `shiftStart`, `shiftEnd`, `isPrimary`
- В UI — возможность добавлять/убирать водителей на путевом листе
- Каждый водитель проходит свой медосмотр

> 💡 Сейчас путевой привязан к одному `driverId`. Нужна связь many-to-many.
...
Путевой лист = центр учёта **всех** затрат рейса:
- **План** — плановые данные (расчётный расход топлива, плановый пробег, нормативы)
- **Чекпоинты** — результаты мед/техосмотра, метки времени прохождения
- **Факт** — фактические затраты:
  - Топливо (литры, стоимость)
  - Платон
  - Стоянки
  - Штрафы
  - Ремонт в пути
  - Прочие расходы
- Итого: **план vs факт** по каждой статье расходов
- Модель `waybill_expenses`: `waybillId`, `category` (fuel/platon/parking/fine/repair/other), `plannedAmount`, `actualAmount`, `description`, `receiptUrl`

> 💡 Сейчас расходы живут отдельно в модуле финансов. Нужно собрать всё на путевом листе.
...
Сейчас прицеп нигде не учтён в системе. В реальности прицеп:
- Имеет свой **госномер**, **VIN**, **тип** (тент/борт/реф/цистерна), **грузоподъёмность**
- Привязывается к тягачу, но может меняться (сцепка/расцепка)
- Имеет свои сроки: **ТО**, **ОСАГО**, **тахограф**
- Проходит свой **предрейсовый техосмотр**
- Новая таблица `trailers` + связь `trips.trailerId`

> 💡 Без этой модели пункт 1 (данные по прицепу в заявке) невозможно реализовать полноценно. Прицеп ≠ ТС, это отдельная сущность со своим жизненным циклом.

### 14. Температурный режим (рефрижераторы)
Добавить в заявку:
- **Температурный режим** (`temperatureMin`, `temperatureMax`, °C) — для рефрижераторных перевозок
- Валидация: при типе кузова `рефрижератор` температура обязательна
- Контроль: факт vs план температуры при доставке (будущее — датчики)

> 💡 Критично для продуктовых и фармацевтических перевозок. Если клиент заказал -18°C, а привезли при -5°C — это рекламация и убытки.

### 15. Тип загрузки/разгрузки
Добавить в заявку:
- **Тип загрузки** (`loadingType`): задняя, боковая, верхняя
- **Наличие гидроборта** (`hydraulicLiftRequired`, boolean)
- Влияет на подбор ТС — не каждая машина может грузиться сверху или имеет гидроборт

> 💡 Нужно также добавить `hasHydraulicLift` в модель `vehicles`, чтобы при назначении ТС автоматически проверять совместимость.

### 16. Доверенность водителя
Водитель перевозит грузы по **доверенности** от перевозчика:
- **Номер доверенности** (`powerOfAttorneyNumber`)
- **Срок действия** (`powerOfAttorneyExpiry`)
- Привязка к таблице `drivers`
- Алерт при истечении (как в предиктивном ТО — `/analytics/maintenance-alerts`)

> 💡 Без доверенности водитель не имеет права принимать/сдавать груз. Контролируется на каждом рейсе.
