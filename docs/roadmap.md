...
- **Дневное расписание диспетчера** — экран-календарь (или timeline) на текущий день
- Показывает все рейсы на выбранную дату: какие ТС загружены, какие свободны
- Визуализация слотов по часам (Gantt-подобный вид или Kanban по времени)
- Фильтрация по дате, водителю, транспорту

> 💡 Нужно определить: это отдельная страница `/dispatch/day` или виджет на дашборде?

### 3. Статус заявки: «Подтверждена → В работе»
- Добавить переход в стейт-машину заявок: `confirmed` → `in_progress`
- **`confirmed`** — клиент подтвердил, можно планировать рейс
- **`in_progress`** — рейс назначен и активно выполняется
- Обновить UI перехода статусов + Kanban-доску

> 💡 Сейчас стейт-машина заявок: `new → assigned → in_transit → delivered → completed / cancelled`. Нужно вставить `confirmed` и `in_progress` в правильное место.

### 4. Инцидент по мед/техосмотру
- Если результат медосмотра или техосмотра **не OK** → автоматически создаётся **инцидент**
- Инцидент блокирует выпуск на линию (водитель/ТС не допускаются к рейсу)
