...
- Tracker sync: statuses below now treat batch 2 free hardening as done; remaining "later" items are external integrations, real operators, pilot evidence, paid services, or deeper approval workflows.
...
|---|---|---|---|---|
| Одна заявка -> несколько рейсов | 100 тонн, машины по 20 тонн | Делить заказ на партии/сегменты, считать остаток, закрывать частями | Есть модель `shipment_lots`, `trip_lot_assignments`, read/write API, smoke 100 тонн -> 2 рейса | Сделано бесплатно, проверено 2026-04-29: load-structure UI показывает партии/назначения и связку заказов/рейсов; больше кейсов можно расширять по мере пилота |
| Один рейс -> несколько заявок | Машина забирает 3 заказа у разных клиентов | Консолидировать партии, вести отдельные документы/статусы/биллинг по каждой заявке | `trip_orders`, route points и модель партий поддерживают несколько заказов в рейсе | Сделано бесплатно, проверено 2026-04-29: multi-order foundation и read-only UI-группировка в досье рейса добавлены |
| Частичная погрузка | План 20 тонн, загрузили 18.7 | Фиксировать факт, причину, фото/подписи, пересчет остатка и тарифа | Есть `shipment_facts` по весу/объему/местам, evidence attachments, GPS, подпись, пересчет lot totals | Сделано бесплатно, проверено 2026-04-29: smoke покрывает фото/подпись/паллеты/evidence; tariff-rule evaluator добавлен для бесплатного расчетного слоя |
...
| Мультистоп | Один рейс с несколькими погрузками/выгрузками | Последовательность точек, SLA по каждой точке, документы по каждой партии | Route points создаются при назначении партии, downtime/SLA факты пишутся по точке, cockpit видит исключения | Сделано бесплатно, проверено 2026-04-29: route sequence UI в досье рейса добавлен; solver/VRP остается внешним будущим слоем |
...
| Поломка в пути | Рейс не может продолжаться | Инцидент, эвакуация/ремонт, перегруз в другую машину, уведомления | Есть API breakdown, blocking cockpit signal, nextActions на ремонт/замену ресурса | Сделано бесплатно, проверено 2026-04-29: dossier next actions открывают repair UI с контекстом рейса/ТС |
| Возврат груза | Получатель отказался или часть возвращается | Обратный рейс/сегмент, новые документы, финансовые последствия | `shipment_facts.factType=return`, discrepancy/refusal, claims и finance adjustments покрывают бесплатный контур возврата | Сделано бесплатно, проверено 2026-04-29: контур возврата есть, в dossier next actions добавлен быстрый переход к return checklist |
| Возврат ТС после рейса | Машина должна сдать документы, пройти осмотр, топливо/одометр | Послерейсовый чеклист и блокировка следующего рейса при проблемах | Есть API post-trip-return, фиксация одометра/топлива/документов/осмотра, блокировка следующего рейса через cockpit | Сделано бесплатно, проверено 2026-04-29: return checklist доступен из dossier next actions; глубокая связка с осмотрами может расширяться позже |
...
| Отказ от подписи | Нужно фиксировать отказ, причину, акт/доказательства и дальнейший статус | Сделано бесплатно, проверено 2026-04-29: signature-refusal API, reason/evidence, rejected status, receipt/history, UI отказа и `/print/signature-refusal/:tripId` |
| Потерянные бумажные документы | Missing docs queue, ответственный, дедлайн, финансовая блокировка | Сделано бесплатно, проверено 2026-04-29: `document_dossier_items`, due/blocking, exception endpoint, close gate, document queue в досье и dispatcher cockpit blocker/next action |
...
| Закрытие рейса без документов | Правило: разрешать с риском или блокировать до сдачи досье | Сделано бесплатно, проверено 2026-04-29: dossier projection, close gate, paper exception, document queue, dossier next actions, dispatcher cockpit и mobile blockers показывают риск |
...
| Сделать досье документов как обязательный close gate | Сделано бесплатно, проверено 2026-04-29: projection + `GET /trips/:id/dossier/close-gate`, document queue, exception/cockpit blockers, dispatcher cockpit и dossier next actions |
