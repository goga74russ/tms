...
> **📍 ТЕКУЩЕЕ СОСТОЯНИЕ (2026-06-10, поздно):** **origin/local `e82faae`, prod `465e58a`**. ВСЕ долги закрыты одной сессией: demo-QA F-01/03/05/09/19 + footgun + 2 прод-бага PDF (шрифт+кириллица) + F-17 (тариф+дата+PDF) + регресс-DEFER 3/3 (finance getNextInvoiceNumber perf → advisory-lock+агрегат; incidents scope → org из FK; 0041 NULL-distinct → миграция 0046 partial-unique). Gate: tsc api+web чисто, 735+200 тестов. Smoke на стенде: devCode, выпуск УПД (finance-perf), PDF-имя upd_ — OK. Стенд пересеян чисто (46 миграций). F-18 = артефакт (разрешён через UI). **Открытых долгов нет.** Прод не трогали (deploy по фразе). (на проде регресс-фиксы 7/11; всё после — QA demo-readiness волна на стенде, прод НЕ трогали, деплой по фразе). Demo-readiness фиксы поверх super-admin gate: **F-15** конкретика чек-листа закрытия (4df5acb), **F-16** бумажное исключение по блокеру ЭТрН + бэкенд upsertItem не откатывает ручной exceptioned + честный F-15 (7ffb20d, верифицирован на стенде). Ключевой вывод F-16: close-gate ADVISORY (/complete его не enforce'ит). НЕ трогали проекцию ЭТрН (распознавание подписанных T01-T06 как satisfying close-req) — compliance, нужен Jurist. Весь операционный happy-path пройден QA через чистый UI. DEFER-хвост регрессий (3): finance perf, incidents scope, 0041 NULL-distinct. Поверх `465e58a`: seed osno+`db:seed:demo` (d1d3643), **F-06** ПЛ+старт из досье (6ebac0a), **F-10** числовой max в генераторах ORD/TRP/WB + seed-форматы (4fa28e9, demo-blocker, верифицирован 3 заявки подряд), **F-04** combobox mousedown+coords (354892c), **F-08** точки маршрута+завершение из досье (ceb728a), **super-admin gate** roles[] в Author finance (7031db0 — DEFER-хвост сокращён до 3). Все 4 P1 demo-readiness закрыты в коде; F-04/F-08/gate НЕ на стенде (ждут пересборки web+api по сигналу QA). DEFER (3): finance perf, incidents scope, 0041 NULL-distinct. Поверх волн 1-7 + миграций 0043/0044/0045 закрыты **7 из 11 регрессий повторного аудита** (`docs/qa/regression-2026-06-04.md`): R-1 force-logout (auth token_version по set-сравнению ролей), R-2 invoice picker (orders inArray + client status=delivered,returned), R-3 data-table selection (stable key), P2 waybills odometer (422+soft-warn), P2 ЭТрН INBOUND (гейт только OUTBOUND), P2 settings org-filter в SQL, P3 osago role-gate. Gate: tsc api+web чисто, 735 unit зелёные. **DEFER (4):** finance getNextInvoiceNumber perf (perf-only, не кусает на пилот-объёме — money-SQL не правим под давлением), super-admin gate invoice-workflow:69 (узкий edge org-less super-admin), incidents scope sprint9 + 0041 NULL-distinct (миграция-флейвор, узкие super-admin/NULL-org края).
...
> **NB волна 6 (fake-INN, origin, НЕ на проде):** деплой требует `NEXT_PUBLIC_CARRIER_*` env + ИНН демо-контрагентов, иначе генерация документов отказывает явно (намеренно). Держим на origin — аудитор видит код-фикс, демо-прод цел.
...
> concurrency-locks, correctness). **Реально FIXED/VERIFIED: ~119/181.** Категории D1-D3, D7 в основном
...
реально закрыто **~119/181**. Оставшиеся ~67 — это находки, которые **нельзя или нельзя безопасно**
закрыть реактивной правкой под дедлайн. По каждой категории — вердикт `DEFER` с причиной:

**R1. Миграции БД (~6 осталось).** ✅ **`mchd_number` per-org unique — СДЕЛАНО** (миг.0043 на проде:
name-agnostic DO-блок по pg_constraint; **dry-run против реального прода с ROLLBACK** подтвердил
drop+create; прод mchd=0 строк → нулевой риск; cross-tenant 409-oracle закрыт). Безопасно, т.к. loosening.
Осталось (**dedup-first**, отдельный заход — там реальный риск существующих нарушений):
`tachograph_records(driver,date,source)` unique; `idx_payments_provider_id` unique;
`invoice_orders` allocated_vat-инвариант; `orders/trips/waybills.number` per-org (nullable org → NULL-distinct);
data-миграция нормализации существующих госномеров (может создать коллизию). Причина: каждая = CREATE INDEX/ALTER +
backfill + проверка на существующие дубли + откат-план. Хастить миграции на проде опасно (можно уронить запись). Код-уровень
...
> **Итог DoD (финал):** ~119/181 **FIXED/VERIFIED** (батчи S/C/SE/M/F/G **на проде** + мульти-агент
