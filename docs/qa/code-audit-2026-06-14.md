...
**Закрыто на 2026-06-15: 18 P1** (легальные A1-A7 + #9 + money #2/#3/#5/#10 + security wave1/2).

Осталось P1: rate-limit bypass (auth), documents docType, fleet PUT (drop fields),
inspections cascade, credentials /test, gosklyuch HMAC callback, confirmationMode
bypass, analytics NPE. **Требуют миграцию БД** (отдельный деплой): import orders.number,
invite-team email, repairs parts catalog, global UNIQUE(code) → per-org. **Требует
продуктового решения:** Telegram deep-link (nonce vs in-app confirm).
