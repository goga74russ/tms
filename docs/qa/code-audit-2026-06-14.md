...
Осталось P1 (нелегальные): rate-limit bypass, billing planId, refund webhook, claim FSM, adr IDOR, documents docType, recordPartialPayment, KPI topDrivers, fleet PUT/GET, inspections cascade, import orders.number, credentials /test, Telegram deep-link, invite-team email, GET /orders customerPrice, createOrder FK, repairs IDOR ×2, parts catalog, global UNIQUE(code).
