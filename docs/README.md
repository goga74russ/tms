# TMS v2 Documentation

Updated: 2026-04-28

This documentation describes the clean v2 workspace in `D:\Ai\TMS-prod`.

## Start Here

- [Product Positioning](./product/positioning.md)
- [Roadmap](./product/roadmap.md)
- [Market Gap Backlog](./product/market-gap-backlog.md)
- [Market Comparison Matrix](./product/market-comparison-matrix.md)
- [Architecture Overview](./architecture/overview.md)
- [Mobile Migration](./mobile/migration.md)
- [ETRN / EPD Roadmap](./compliance/etrn-epd-roadmap.md)
- [Deployment](./operations/deployment.md)
- [Release Gate](./operations/release-gate.md)
- [Security](./operations/security.md)

## Documentation Rules

- This `docs/` folder is the v2 source of truth.
- Old documentation from `D:\Ai\TMS\docs` is archive material until manually promoted here.
- If runtime code and documentation disagree, the code wins and docs must be updated.
- Market claims must distinguish internal foundation from legally complete external EDO/EPD exchange.
- [Russian road transport process map](./product/russian-road-transport-process-map.md)
